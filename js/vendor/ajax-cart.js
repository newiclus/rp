...
var ajaxCart = {
			...
			success: function(jsonData)
			{
				...
				if (callerElement && callerElement != null)
					$(callerElement).parents().each( function() {
						...
						if ($(this).find('a').is('.product_image')) {img = $("a.product_image img", this);}

						else {
							if($(this).find('a').is('.product_img_link')) {img = $("a.product_img_link img",this);}
							else img = $("a img", this);
						}
							ajaxCart.imageTransfer(elementToTransfert,img,'0');
						}
					});
				else{
					elementToTransfert = $(addedFromProductPage ? 'div#image-block' : ('.ajax_block_product_id_' + idProduct) );
						if(addedFromProductPage) {
							ajaxCart.imageTransfer('#image-block','#bigpic','0.3');
						}
					}
	...
	imageTransfer : function(container_block,image_block,ratio){
		...
		if(ratio==0){
			...
			.animate({ marginLeft: gotoX, marginTop: gotoY}, 1200, function() {
				$(this).remove();
				});	  
		}
			else{	
			var newImageWidth   = $(container_block).width() * parseFloat(ratio);
			var newImageHeight  = $(container_block).height() * parseFloat(ratio);
			
			$(image_block)
			.clone()
			.prependTo(container_block)
			.css({'position' : 'absolute','width': newImageWidth, 'height': newImageHeight})
